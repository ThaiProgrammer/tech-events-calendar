...
function processData(
  ...
  item('`topics`', () => {
    ...
    function topicValid(v) {
      ...
      if (v === 'TODO') return 'topic should not be TODO'
      return true
    }
  })
  ...
  item('**title** (in markdown body)', () => {
    element(markdownBody)
      ...
      .map(b => b.title)
      .condition('title is present', t => !!t && t !== 'TODO')
      .ok(t => {
        event.title = t
      })
  })
  ...
  item('`location`', () => {
    element(attributes.location)
      ...
      .conditions([
        [
          '`title` is a string',
          l => l && l !== 'TODO' && typeof l.title === 'string'
        ],
        [
          '`url` is an optional Google Maps URL or TBD',
          l => {
            const { url } = l
            if (!url) {
              return true
            ...
            if (!isGoogleMapURL(url)) {
              return 'not a Google Maps URL (should start with `https://www.google.com/maps/place` or `https://goo.gl/maps/` for shortened one)'
            }
            return true
          }
        ],
  ...
  item('**summary** (in markdown body)', () => {
    element(markdownBody)
      ...
      .map(b => b.summary)
      .condition('is present', s => !!s && s !== 'TODO')
      .condition(
        'is not longer than 280 chars',
        s => (s.length <= 280 ? true : `too long (${s.length})`)
      )
      .ok(s => {
        event.summary = s
      })
  ...
  item('**description** (in markdown body)', () => {
    element(markdownBody)
      ...
      .map(b => b.description)
      .condition('is valid', s => (s === 'TODO' ? 'should not be TODO' : true))
      .ok(s => {
        event.description = s
      })
  })
      ...
      return element(v).conditions([
        ...
        [
          ...
          () => {
            ...
            if (v.price === 'TODO') {
              return '`price` should not be TODO'
            }

...
function processData ({ body, attributes }, event, { item, element, combine }, options) {
  // id
  item('`id`', () => {
    element(attributes.id)
      .condition('is present', x => !!x)
      .condition('is a string', x => typeof x === 'string')
      .condition('contains only lowercase characters, numbers, or hyphens', x => /^[a-z0-9-]+$/.test(x))
      .ok(x => {
        event.id = x
      })
  })
    ...
    function checkValidLink (v) {
      ...
      return element(v)
        .conditions([
          ...
          [ '`detail` is an optional string', () => !v.detail || (typeof v.detail === 'string') ],
  ...
  item('`resources`', () => {
    ...
    element(attributes.resources)
      .condition('is an array', x => Array.isArray(x))
      .flatMap(a => combine(a.map(
        (v, i) => item(`item ${i}`, () => checkValidResource(v))
      )))
      .ok(resources => {
